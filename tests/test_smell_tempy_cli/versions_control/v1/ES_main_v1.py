...
def get_selected_test_file(lista, tempdir): # nao usa
...
>>> O que é a lista?? \n
{lista} \n\n
seu tipo é {type(lista)}\n
agora o tempdir:
{tempdir} \n\n
e sua tipagem é: {type(tempdir)}
"""
	print(data)
	if True:
		all_logs, projects = [], []
		...
		# webbrowser é pra abrir automaticamente
		#webbrowser.open(url,new=1)
		sys.exit(0)
...
def test_file_selection_window(nomes,paths,tempdir):
	"""
	Aqui é onde voce seleciona o total de testes que vc quer e depois aperta ok.
	No caso vai ser o btn2. Windows_3
	""" 
	...
	# newWindow.protocol('WM_DELETE_WINDOW', lambda: close_window_confirmation(newWindow)) # nao usa
	...
	#btn1 = tk.Button(newWindow, text='Select', command=lambda: get_selected_test_file(lista, tempdir)).pack(fill=tk.X) # nao é pra usar
	#btn2 = tk.Button(newWindow, text='Select All', command=lambda: get_all_test_file(lista,tempdir)).pack(fill=tk.X)
	get_all_test_file(lista,tempdir)
	#btn3 = tk.Button(newWindow, text='Cancel', command=lambda: close_window(newWindow)).pack(fill=tk.X) # nao é pra usar

def generate_test_file_list_log(files,nomes,paths, tempdir):
	"""
	Gera um log dos resultados encontrados dentro de 'search_test_file.
	
	:param files: numero de arquivos encontrados
	:param nomes: nome dos arquivos de teste
	:param paths: path dos arquivos de teste
	:param tempdir: diretorio root
	"""
	if (files >= 1):
...
def search_test_file(tempdir):
	"""
	Aqui é feito a busca de todos os arquivos de testes a partir desse diretorio root.
	"""
...
def select_directory():
	"""
	Este metodo é mostra as opções de diretorios pra voce escolher a pasta.
	O código varre os arquivos de teste e lhe retorna a quantidade.
	Depois disso vai pra outra janela, pra voce escolher quais os testes que quer
	gerar a qualidade. WINDOW_2.

	:return: TEMPDIR = TEMPORARY DIRECTORY
	"""
	currdir = os.getcwd()
...
#===================================================================================================================
"""
AQUI SE A PASTA REPORT NÃO EXISTIR, CRIA UM NA HORA.
"""
p = Path("./report/")
if p.exists() == False:
	os.mkdir("./report/")
...
"""
AQUI É ONDE VOCE SETA O DIRETORIO ONDE VAI ENCONTRAR OS ARQUIVOS DE TESTE. WINDOW_1
>>> def select_directory ...
"""
